 ...
 * You can donate Feathercoins to address 6wf5ebYFfviaTkeEWGSnW92ahkmVSZ2uuh   
...
/**
 * Feathercoin utility functions class
...
define("FEATHERCOIN_ADDRESS_VERSION", "0E");// this is a hex byte
/**
 * Exception class for FeathercoinClient
 *
 * @author Mark Mikkelson
 * Based on Mike Gogulski's Bitcoin Exception class
 *  http://www.madcapsule.com/
...
/**
 * Feathercoin client class for JSON-RPC-HTTP[S] calls
 *
 *
 * @version 0.0.1
 * @author Mark Mikkelson
 ...
 */
  ...
  private function decodeBase58($base58) {
    ...
    for ($i = 0; $i < strlen($base58); $i++) {
      $current = (string) strpos(Feathercoin::$base58chars, $base58[$i]);
  ...
  /**
   * Convert a 160-bit Feathercoin hash to a Feathercoin address
   *
   * @author theymos
   * @param string $hash160
   * @param string $addressversion
   * @return string Feathercoin address
   * @access public
   */
  public static function hash160ToAddress($hash160, $addressversion = FEATHERCOIN_ADDRESS_VERSION) {
  ...
  /**
   * Convert a Feathercoin address to a 160-bit Feathercoin hash
   *
   * @author theymos
   * @param string $addr
   * @return string Feathercoin hash
   * @access public
   */
  ...
  /**
   * Determine if a string is a valid Feathercoin address
   *
   * @author theymos
   ...
   * @access public
   */
  public static function checkAddress($addr, $addressversion = FEATHERCOIN_ADDRESS_VERSION) {
    $addr = self::decodeBase58($addr);
    if (strlen($addr) != 50) {
      return false;
    }
  ...
  /**
   * Convert the input to its 160-bit Feathercoin hash
   *
   * @param string $data
   * @return string
   * @access private
  ...
  /**
   * Convert a Feathercoin public key to a 160-bit Feathercoin hash
   *
   * @param string $pubkey
   * @return string
   * @access public
 ...
 * http://www.madcapsule.com/ 
 */
class FeathercoinClientException extends ErrorException {
...
/**
 * Feathercoin client class for access to a Feathercoin server via JSON-RPC-HTTP[S]
 *
 * Implements the methods documented at https://www.feathercoin.org/wiki/doku.php?id=api
 *
 * @version 0.1.0
 * @author Mark Mikkelson
 ...
 * http://www.madcapsule.com/ 
 */
class FeathercoinClient extends jsonrpc_client {

  /**
   * Create a jsonrpc_client object to talk to the feathercoin server and return it,
   * or false on failure.
   *
   * @param string $scheme
   *  "http" or "https"
   * @param string $username
   *  User name to use in connection the Feathercoin server's JSON-RPC interface
   ...
   * @access public
   * @throws FeathercoinClientException
   */
  public function __construct($scheme, $username, $password, $address = "localhost", $port = 8332, $certificate_path = '', $debug_level = 0) {
    $scheme = strtolower($scheme);
    if ($scheme != "http" && $scheme != "https")
      throw new FeathercoinClientException("Scheme must be http or https");
    if (empty($username))
      throw new FeathercoinClientException("Username must be non-blank");
    if (empty($password))
      throw new FeathercoinClientException("Password must be non-blank");
    $port = (string) $port;
    if (!$port || empty($port) || !is_numeric($port) || $port < 1 || $port > 65535 || floatval($port) != intval($port))
      throw new FeathercoinClientException("Port must be an integer and between 1 and 65535");
    if (!empty($certificate_path) && !is_readable($certificate_path))
      throw new FeathercoinClientException("Certificate file " . $certificate_path . " is not readable");
  ...
  /**
   * Test if the connection to the Feathercoin JSON-RPC server is working
   *
  ...
  public function can_connect() {
    try {
      $r = $this->getinfo();
    } catch (FeathercoinClientException $e) {
      return $e->getMessage();
    }
    return true;
  }
  ...
  /**
   * Convert a Feathercoin server query argument to a jsonrpcval
   *
  ...
  public function query($message) {
    if (!$message || empty($message))
      throw new FeathercoinClientException("Feathercoin client query requires a message");
    ...
    if ($response->faultCode()) {
      throw new FeathercoinClientException($response->faultString());
    }
    return php_xmlrpc_decode($response->value());
  }

  /*
   * The following functions implement the Feathercoin RPC API as documented at https://www.feathercoin.org/wiki/doku.php?id=api
   */
  ...
  public function backupwallet($destination) {
    if (!$destination || empty($destination))
      throw new FeathercoinClientException("backupwallet requires a destination");
   ...
   * @return float Feathercoin balance
   * @throws FeathercoinClientException
   */
  public function getbalance($account = NULL, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('getbalance requires a numeric minconf >= 0');
    if ($account === NULL)
  ...
  /**
   * Tell Feathercoin server to generate Bitcoins or not, and how many processors
   * to use.
   *
  ...
  public function setgenerate($generate = TRUE, $maxproc = -1) {
    if (!is_numeric($maxproc) || $maxproc < -1)
      throw new FeathercoinClientException('setgenerate: $maxproc must be numeric and >= -1');
  ...
  public function getaccount($address) {
    if (!$address || empty($address))
      throw new FeathercoinClientException("getaccount requires an address");
  ...
  public function getlabel($address) {
    if (!$address || empty($address))
      throw new FeathercoinClientException("getlabel requires an address");
  ...
  public function setaccount($address, $account = "") {
    if (!$address || empty($address))
      throw new FeathercoinClientException("setaccount requires an address");
  ...
  public function setlabel($address, $label = "") {
    if (!$address || empty($address))
      throw new FeathercoinClientException("setlabel requires an address");
   ...
   * @return float Feathercoin total
   * @throws FeathercoinClientException
   */
  public function getreceivedbyaddress($address, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('getreceivedbyaddress requires a numeric minconf >= 0');
    if (!$address || empty($address))
      throw new FeathercoinClientException("getreceivedbyaddress requires an address");
  ...
  public function getreceivedbyaccount($account, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('getreceivedbyaccount requires a numeric minconf >= 0');
    if (!$account || empty($account))
      throw new FeathercoinClientException("getreceivedbyaccount requires an account");
  ...
  public function getreceivedbylabel($label, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('getreceivedbylabel requires a numeric minconf >= 0');
    if (!$label || empty($label))
      throw new FeathercoinClientException("getreceivedbylabel requires a label");
  ...
  public function listreceivedbyaddress($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('listreceivedbyaddress requires a numeric minconf >= 0');
  ...
  public function listreceivedbyaccount($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('listreceivedbyaccount requires a numeric minconf >= 0');
  ...
  public function listreceivedbylabel($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('listreceivedbylabel requires a numeric minconf >= 0');
   ...
   *
   * @param string $address Destination Feathercoin address or IP address
  ...
  public function sendtoaddress($address, $amount, $comment = NULL, $comment_to = NULL) {
    if (!$address || empty($address))
      throw new FeathercoinClientException("sendtoaddress requires a destination address");
    if (!$amount || empty($amount))
      throw new FeathercoinClientException("sendtoaddress requires an amount to send");
    if (!is_numeric($amount) || $amount <= 0)
      throw new FeathercoinClientException("sendtoaddress requires the amount sent to be a number > 0");
  ...
  /**
   * Check that $address looks like a proper Feathercoin address.
   *
   * @param string $address String to test for validity as a Feathercoin address
  ...
  public function validateaddress($address) {
    if (!$address || empty($address))
      throw new FeathercoinClientException("validateaddress requires a Feathercoin address");
  ...
  public function gettransaction($txid) {
    if (!$txid || empty($txid) || strlen($txid) != 64 || !preg_match('/^[0-9a-fA-F]+$/', $txid))
      throw new FeathercoinClientException("gettransaction requires a valid hexadecimal transaction ID");
  ...
  public function move($fromaccount = "", $toaccount, $amount, $minconf = 1, $comment = NULL) {
    ...
    if (!$amount || !is_numeric($amount) || $amount <= 0)
      throw new FeathercoinClientException("move requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('move requires a numeric $minconf >= 0');
   ...
   * @param string $toaddress Feathercoin address to send to
  ...
  public function sendfrom($account, $toaddress, $amount, $minconf = 1, $comment = NULL, $comment_to = NULL) {
    if (!$account || !$toaddress || empty($toaddress) || !$amount || !is_numeric($amount) || $amount <= 0)
      throw new FeathercoinClientException("sendfrom requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('sendfrom requires a numeric $minconf >= 0');
    if (!$comment && !$comment_to)
  ...
  /**
   * Return the current feathercoin address for receiving payments to $account.
   * The account and address will be created if $account doesn't exist.
   *
   * @param string $account Account name
   * @return string Feathercoin address for $account
   * @throws FeathercoinClientException
   * @since 0.3.18
   */
  public function getaccountaddress($account) {
    if (!$account || empty($account))
      throw new FeathercoinClientException("getaccountaddress requires an account");
   ...
   * @param string $account
   * @return array
   *    A simple array of Feathercoin addresses associated with $account, empty
   *    if the account doesn't exist.
   * @throws FeathercoinClientException
   */
  public function getaddressesbyaccount($account) {
    if (!$account || empty($account))
      throw new FeathercoinClientException("getaddressesbyaccount requires an account");
  ...
  if (!$account) $account = "";
    ...
    if (!is_numeric($count) || $count < 0)
      throw new FeathercoinClientException('listtransactions requires a numeric count >= 0');
    if (!is_numeric($from) || $from < 0)
      throw new FeathercoinClientException('listtransactions requires a numeric from >= 0');
  ...
  public function sendmany($fromAccount, $sendTo, $minconf = 1, $comment=NULL) {
    if (!$fromAccount || empty($fromAccount))
      throw new FeathercoinClientException("sendmany requires an account");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FeathercoinClientException('sendmany requires a numeric minconf >= 0');
